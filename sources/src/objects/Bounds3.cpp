...
const Point3& Bounds3::operator[](int i) const {
    if (i == 0) return pMin;
    if (i == 1) return pMax;
    throw std::invalid_argument("invalid access to bound3 " + std::to_string(i));
}

Point3& Bounds3::operator[](int i) {
    if (i == 0) return pMin;
    if (i == 1) return pMax;
    throw std::invalid_argument("access bounds must be 0 or 1");
}

Point3 Bounds3::corner(int c) const {
    return Point3{
        (*this)[(c & 1)](0),
        (*this)[(c & 2)](1),
        (*this)[(c & 4)](2)
...
Bounds3 Bounds3::get_union(const Bounds3& b, const Point3& p) {
    return Bounds3 {
        Point3 {
...
Bounds3 Bounds3::get_union(const Bounds3& b1, const Bounds3& b2) {
    return Bounds3 {
        Point3 {
...
Bounds3 Bounds3::make_union(const Bounds3& b1, const Bounds3& b2) {
    return Bounds3 {
        Point3 {
            std::min(b1.pMin(0), b2.pMin(0)),
            std::min(b1.pMin(1), b2.pMin(1)),
...
Bounds3 Bounds3::get_intersection(const Bounds3& b1, const Bounds3& b2) {
     return Bounds3 {
        Point3 {
...
bool Bounds3::inside(const Point3& p, const Bounds3& b) const {
...
Bounds3 Bounds3::expand(const Bounds3& b, RealValue delta) {
    return Bounds3(
            b.pMin - Vec3{delta, delta, delta},
            b.pMax + Vec3{delta, delta, delta});
}

inline Vec3 Bounds3::diagonal() const { return pMax - pMin; }

RealValue Bounds3::surface_area() const {
    Vec3 d = diagonal();
    return 2 * (d(0) * d(1) + d(0) * d(2) + d(1) * d(2));
}

RealValue Bounds3::volume() const {
    Vec3 d = diagonal();
    return d(0) * d(1) * d(2); 
}
...
void Bounds3::bounding_sphere(Point3 * center, float * radius) const {
    *center = (pMin + pMax) / 2;
    *radius = inside(*center, *this) ? distance(*center, pMax) : 0;
}

bool Bounds3::intersect(const Ray& ray, float *hitt0, float* hitt1) const {
...
bool Bounds3::intersect(const Ray& ray, const Vec3 &inv_dir, 
